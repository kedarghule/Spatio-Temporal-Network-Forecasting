...
class SpatioTemporalEmbedding(nn.Module):
    ...
    def __init__(
        self, D: int, bn_decay: float, steps_per_day: int = 288, use_bias: bool = True
    ):
    ...
    def forward(
        self, SE: torch.FloatTensor, TE: torch.FloatTensor, T: int = 288
    ) -> torch.FloatTensor:
        """
...
def get_spatial_embedding(x, D):
    seq_len = x.shape[1]
    input_dim = D
    pe = x.new_zeros(seq_len,1,input_dim)
...
class TemporalAttention(nn.Module):
    ...
    def forward(
        ...
        value = self._fully_connected_v(X)
        #print(self._K)
        #print(torch.split(query, self._K, dim=-1)[0].shape)
        #temp = torch.split(query, self._d, dim=-1)
        #print(len(temp))
